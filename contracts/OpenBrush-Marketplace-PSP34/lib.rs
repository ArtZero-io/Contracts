...
pub mod artzero_marketplace_psp34 {
    ...
    impl EnumerableMapping {
        pub fn insert(
            ...
            self.id_to_index
                .insert((nft_contract_address, seller, id), index);
            self.index_to_id
                .insert((nft_contract_address, seller, index), id);
        }
        ...
        pub fn remove(
            ...
            let index = self
                .id_to_index
                .get((nft_contract_address, seller, id))
            ...
            if last_index != &index {
                let last_id = self
                    ...
                    .ok_or(PSP34Error::TokenNotExists)?;
                self.index_to_id
                    .insert((nft_contract_address, seller, &index), &last_id);
                self.id_to_index
                    .insert((nft_contract_address, seller, &last_id), &index);
            }

            self.index_to_id
                .remove((nft_contract_address, seller, &last_index));
        ...
        pub fn get_by_index(
            ...
            self.index_to_id
                .get((nft_contract_address, seller, index))
                .ok_or(PSP34Error::TokenNotExists)
        }
        pub fn get_by_id(
            &self,
    ...
    impl ArtZeroMarketplacePSP34 {
        ...
        pub fn list(
            ...
            assert!(allowance);
            let is_active = CollectionRef::is_active(
            ...
            assert!(is_active);
            {
                ...
                self.manager.sale_tokens_ids_last_index.insert(
        ...
        pub fn buy(&mut self, nft_contract_address: AccountId, token_id: Id) -> Result<(), Error> {
            ...
            assert!(CollectionRef::is_active(
                &self.manager.collection_contract_address,
                nft_contract_address
            )); //collection must be active
            let contract_type = CollectionRef::get_contract_type(
                &self.manager.collection_contract_address,
                nft_contract_address,
            );
                ...
                .is_some()
            {
                ...
                self.manager.bidders.insert(
                    &(nft_contract_address, seller, token_id.clone()),
                    &clear_bidders,
                );
            }
            //Calculate fee
            ...
            self.manager.current_profit = self
                ...
                .checked_add(platform_fee_after_discount)
                .unwrap();
            let royal_fee_rate = CollectionRef::get_royal_fee(
                &self.manager.collection_contract_address,
                nft_contract_address,
            );
            let royal_fee = price
            ...
            let collection_owner = CollectionRef::get_collection_owner(
                &self.manager.collection_contract_address,
                nft_contract_address,
            );
            ...
            if royal_fee > 0 {
                assert!(self
                    .env()
                    .transfer(collection_owner.unwrap(), royal_fee)
                    .is_ok());
            }
            ...
            self.manager
                .volume_by_user
                .insert(&seller, &user_volume_unwrap);
            self.update_listed_token_by_collection_address(nft_contract_address, false);
            //Send NFT to Buyer
            assert!(Psp34Ref::transfer(
                &nft_contract_address,
        ...
        pub fn bid(&mut self, nft_contract_address: AccountId, token_id: Id) -> Result<(), Error> {
            ...
            assert!(self
                .manager
                .sale_tokens_ids
                .get_by_id(&Some(nft_contract_address), &Some(seller), &token_id)
                .is_ok());
                ...
                .is_some()
            {
                ...
                for index in 0..bidders_length {
                    if index == bid_index as usize {
                        ...
                        let collection_volume =
                            self.manager.volume_by_collection.get(&nft_contract_address);
                        let user_volume = self.manager.volume_by_user.get(&seller);
                        let mut user_volume_unwrap = 0;
                        let mut collection_volume_unwarp = 0;
                        if collection_volume.is_some() {
                        ...
                        if platform_fee > platform_fee_after_discount {
                            ...
                            self.manager.total_volume =
                                self.manager.total_volume.checked_add(volume).unwrap();
                            collection_volume_unwarp =
                                collection_volume_unwarp.checked_add(volume).unwrap();
                            user_volume_unwrap = user_volume_unwrap.checked_add(volume).unwrap();
                        } else {
                            self.manager.total_volume =
                                self.manager.total_volume.checked_add(price).unwrap();
                            collection_volume_unwarp =
                                collection_volume_unwarp.checked_add(price).unwrap();
                            user_volume_unwrap = user_volume_unwrap.checked_add(price).unwrap();
                        }
                        self.manager
                            .volume_by_collection
        ...
        #[ink(message)]
        #[modifiers(only_owner)]
        pub fn set_collection_contract_address(
            &mut self,
            collection_contract_address: AccountId,
        ) -> Result<(), Error> {
            self.manager.collection_contract_address = collection_contract_address;
        ...
        //Set listed token
        pub fn update_listed_token_by_collection_address(
            &mut self,
            nft_contract_address: AccountId,
            mode: bool,
        ) {
        ...
        #[ink(message)]
        #[modifiers(only_owner)]
        pub fn set_staking_contract_address(
            &mut self,
            staking_contract_address: AccountId,
        ) -> Result<(), Error> {
            self.manager.staking_contract_address = staking_contract_address;
        ...
        #[ink(message)]
        pub fn get_listed_token_count_by_collection_address(
            &self,
            collection_contract_address: AccountId,
        ) -> u64 {
        ...
        pub fn get_for_sale_token_id(
            ...
            self.manager
                .sale_tokens_ids
                .get_by_index(&Some(nft_contract_address), &Some(user_account), &index)
                .unwrap()
        }
        ///Get all token ids currently for sale by a collection (nft_contract_address,user_account)
        #[ink(message)]
        pub fn total_tokens_for_sale(
            &self,
            nft_contract_address: AccountId,
            user_account: AccountId,
        ) -> u128 {
        ...
        fn apply_discount(&self, staker: AccountId, input_fee: Balance) -> Balance {
            let staked_amount = StakingRef::get_total_staked_by_account(
                &self.manager.staking_contract_address,
                staker,
            );
            ...
            for index in 0..length {
                if staked_amount >= self.manager.staking_discount_criteria[index] as u32 {
                    return (input_fee
                        * (10000 - self.manager.staking_discount_rate[index] as u128))
                        / 10000;
                }
            }
            return input_fee;
